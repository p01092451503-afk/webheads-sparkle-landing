    ...
    <div className="min-h-screen bg-background">
      ...
      <section className="relative min-h-[76vh] flex items-center pt-20 pb-14 overflow-hidden" style={{ background: "var(--hero-bg)" }}>
        ...
        <div className="absolute inset-0 pointer-events-none select-none flex items-center justify-center" style={{ opacity: 0.85 }}>
          <HeroFloatingVisual><DrmHeroVisual /></HeroFloatingVisual>
        </div>
        <div className="container mx-auto px-6 py-24 relative z-10 lg:pl-[10%]">
          <div className="max-w-xl">
            ...
            <h1 className="text-4xl lg:text-[3.2rem] font-black leading-tight mb-5 tracking-tight text-foreground">{t("drm.hero.title")}<br /><span className="hero-gradient-text-accent hero-title-highlight">{t("drm.hero.titleHighlight")}</span></h1>
            <p className="text-lg leading-relaxed mb-8 max-w-md text-muted-foreground">{t("drm.hero.desc")}</p>
            <div className="flex gap-3 flex-wrap">
              <a href="#contact" className="hero-cta-primary px-7 py-3.5 rounded-2xl font-bold text-sm bg-foreground text-background">{t("drm.hero.cta1")}</a>
              <a href="#solutions" className="hero-cta-secondary px-7 py-3.5 rounded-2xl font-bold text-sm border border-border text-foreground bg-background/80 dark:bg-muted/80 backdrop-blur-sm">{t("drm.hero.cta2")}</a>
            </div>
          </div>
        </div>
      </section>
