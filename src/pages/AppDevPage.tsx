...
const testimonials = [
  { name: "고민수", role: "모바일 사업팀", org: "V 사이버대학교", rating: 3, date: "2024.10", period: "이용 2년차", content: "iOS랑 Android 앱 둘 다 만들었는데, 웹에서 수강하다가 앱으로 넘어가도 이어서 들을 수 있어요. 학습자분들이 앱 편하다고 많이 좋아하십니다." },
  { name: "남지윤", role: "교육기획 팀장", org: "W 대기업 교육센터", rating: 5, date: "2025.01", period: "이용 5년차", content: "Push 알림 보내기 시작하면서 앱 다시 들어오는 비율이 확실히 높아졌어요. 출장 많은 임직원들은 오프라인 다운로드 기능을 제일 좋아합니다." },
  { name: "황도윤", role: "대표", org: "X 스타트업", rating: 4, date: "2024.08", period: "이용 3년차", content: "MVP를 한 달도 안 돼서 앱스토어에 올렸어요. 이후에 기능 추가하는 것도 유연하게 돼서 좋고, 앱스토어 심사 리젝 대응까지 해주셔서 진짜 편했습니다." },
];

const lmsFeatures = [
  { icon: Tablet, title: "웹헤즈 LMS 완벽 연동", desc: "웹헤즈 이러닝 솔루션과 REST API·WebSocket으로 실시간 양방향 연동합니다.", tags: ["REST API", "WebSocket", "실시간 동기화"] },
  { icon: Bell, title: "학습 독려 PUSH 알림", desc: "FCM·APNs를 통해 학습 독려, 과제 마감 알림을 학습자별 최적 시점에 자동 발송합니다.", tags: ["FCM", "APNs", "개인화 알림"] },
  { icon: Lock, title: "모바일 DRM 영상 보안", desc: "Widevine(Android)·FairPlay(iOS) DRM으로 스트리밍 영상의 불법 녹화·캡처를 원천 차단합니다.", tags: ["Widevine", "FairPlay", "AES-128"] },
  { icon: Wifi, title: "오프라인 학습 지원", desc: "네트워크가 불안정한 환경에서도 강의 콘텐츠를 로컬 캐싱하여 끊김 없는 학습을 지원합니다.", tags: ["로컬 캐싱", "오프라인 모드", "자동 동기화"] },
  { icon: BarChart3, title: "학습 분석 & 진도 관리", desc: "학습 시간, 진도율, 퀴즈 정답률 등 30개 이상의 지표를 앱 내 대시보드로 시각화합니다.", tags: ["30+ 지표", "실시간 차트", "관리자 대시보드"] },
  { icon: ShieldCheck, title: "보안 & SSO 연동", desc: "OAuth 2.0·SAML 2.0 기반 SSO로 웹헤즈 LMS 계정 하나로 앱에도 즉시 로그인합니다.", tags: ["OAuth 2.0", "SAML SSO", "루팅 감지"] },
];

const otherApps = [
  { icon: Globe, title: "커머스 & 마켓플레이스", desc: "상품 등록·결제·배송 추적·리뷰까지 풀스택 커머스 앱을 구축합니다.", tags: ["PG 결제", "정산 자동화", "리뷰"] },
  { icon: Zap, title: "예약 & 스케줄 관리", desc: "강의실·회의실·의료 예약 등 다양한 예약 관리 앱을 개발합니다.", tags: ["예약 시스템", "알림 자동화", "캘린더"] },
  { icon: Smartphone, title: "사내 업무 & 그룹웨어", desc: "근태 관리, 전자결재, 사내 공지 등 조직에 맞는 사내 앱을 맞춤 제작합니다.", tags: ["근태 관리", "전자결재", "ERP 연동"] },
  { icon: Code2, title: "커뮤니티 & 소셜 플랫폼", desc: "피드, 채팅, 팔로우 등 SNS형 커뮤니티 기능부터 전문 커뮤니티 앱까지 지원합니다.", tags: ["피드", "실시간 채팅", "알림"] },
];
...
const faqs = [
  { q: "iOS와 Android를 동시에 개발하면 비용이 두 배인가요?", a: "아닙니다. React Native 또는 Flutter 크로스플랫폼 개발 시 단일 코드베이스로 iOS·Android를 동시에 개발하므로 네이티브 개별 개발 대비 30~40% 비용을 절감할 수 있습니다." },
  { q: "앱스토어/구글플레이 등록도 대행해 주시나요?", a: "네, App Store·Google Play 등록 전 과정(스크린샷 제작, 메타데이터 작성, 심사 대응, 리젝 시 수정)을 대행합니다." },
  { q: "기존 웹 LMS를 앱으로 전환할 수 있나요?", a: "네, 웹헤즈 LMS를 사용 중이라면 REST API 기반으로 기존 데이터를 그대로 연동하여 앱을 빠르게 구축할 수 있습니다. 평균 4주 내 MVP를 제공합니다." },
  { q: "앱 출시 후 유지보수도 가능한가요?", a: "네, 앱 업데이트, 버그 수정, OS 업데이트 대응, 기능 추가 등 지속적인 유지보수 서비스를 제공합니다. 별도 유지보수 계약으로 안정적인 운영을 지원합니다." },
  { q: "오프라인 학습 기능이 모든 콘텐츠에 적용되나요?", a: "DRM 정책에 따라 오프라인 다운로드 허용 여부를 콘텐츠별로 설정할 수 있습니다. 다운로드된 콘텐츠도 DRM 암호화가 유지됩니다." },
];
...
export default function AppDevPage() {
  return (
    <div className="min-h-screen bg-background">
      <SEO
        title="어플리케이션 개발"
        description="웹헤즈 LMS와 완벽 연동되는 iOS·Android 앱 개발."
        keywords="이러닝 앱 개발, LMS 앱 개발, iOS 앱, Android 앱, React Native, Flutter"
        path="/app-dev"
        jsonLd={{
      ...
      <section className="py-28 bg-background">
        <div className="container mx-auto px-6 max-w-5xl">
          <div className="mb-16">
            ...
            <p className="text-muted-foreground mt-4 text-base">웹헤즈 LMS를 사용 중이라면 별도 구축 없이 앱에서도 동일한 학습 환경을 제공할 수 있습니다.</p>
          </div>
      ...
      <ServiceMidCTA heading="우리에게 필요한 앱이 궁금하신가요?" description="기관 특성에 맞는 앱 개발 방안을 무료로 제안드립니다." ctaText="개발 상담 신청하기" />
      ...
      <section className="py-24 bg-background border-t border-border">
        <div className="container mx-auto px-6 max-w-5xl">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
            {[
              { icon: Smartphone, title: "iOS / Android 네이티브", desc: "Swift(iOS), Kotlin(Android)으로 각 플랫폼 가이드라인을 완벽히 준수하는 네이티브 앱을 개발합니다.", tags: ["Swift", "Kotlin", "네이티브 UI"] },
              { icon: RefreshCw, title: "하이브리드 / 크로스플랫폼", desc: "React Native·Flutter로 단일 코드베이스에서 iOS·Android를 동시에 개발합니다.", tags: ["React Native", "Flutter", "비용 절감"] },
              { icon: Settings2, title: "앱스토어 배포 & 유지관리", desc: "App Store·Google Play 심사 대응부터 배포까지 전 과정을 대행합니다.", tags: ["App Store", "Google Play", "CI/CD"] },
            ].map((f) => (
              <div key={f.title} className="rounded-2xl p-7 bg-secondary hover:bg-muted transition-colors duration-200 flex flex-col gap-3">
                <div className="w-10 h-10 rounded-xl flex items-center justify-center bg-background shadow-sm">
                  <f.icon className="w-5 h-5 text-primary" />
      ...
      <ServiceFAQ faqs={faqs} serviceName="앱 개발 서비스" />
