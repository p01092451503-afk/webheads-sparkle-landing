    ...
    <div className="min-h-screen" style={{ background: "hsl(252, 30%, 97%)" }} data-page="overview">
      ...
      <section className="relative pt-36 pb-24 overflow-hidden">
        ...
        <div className="container mx-auto px-6 max-w-4xl relative z-10 text-center">
          ...
          <h1 className="text-3xl sm:text-4xl lg:text-[2.8rem] leading-tight tracking-tight text-white whitespace-pre-line" style={{ fontWeight: 900 }}>
                ...
                <div key={card.key} className="rounded-2xl bg-white border border-border overflow-hidden shadow-sm">
                  <div className="p-6">
                    <div className="flex items-start gap-4">
                      <div className="w-10 h-10 rounded-xl flex items-center justify-center shrink-0" style={{ background: card.accent + "1a", color: card.accent }}>
                        <Icon className="w-5 h-5" />
                      </div>
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-2">
                          <h3 className="font-bold text-sm text-foreground">{svc.title}</h3>
                          <a href={card.path} className="text-[10px] font-semibold flex items-center gap-0.5 shrink-0" style={{ color: card.accent }}>
                        ...
                        <div className="flex flex-wrap gap-1.5">
                          {svc.highlights.map((h, j) => (
                            <span key={j} className="px-2 py-0.5 rounded text-[10px] font-medium" style={{ background: card.accent + "12", color: card.accent }}>{h}</span>
                          ))}
                        </div>
                      </div>
                    </div>
                    ...
                    {plans && plans.length > 0 && (
                      <div className="mt-5 pt-4 border-t border-border">
                        <p className="text-[10px] font-bold text-muted-foreground tracking-wider uppercase mb-3">요금제</p>
                        <div className={`grid gap-2 ${plans.length === 4 ? "grid-cols-2 lg:grid-cols-4" : "grid-cols-1 sm:grid-cols-3"}`}>
                          {plans.map((plan: any, pi: number) => (
                            <div key={pi} className={`rounded-lg p-3 border ${plan.highlight ? "border-[hsl(255,75%,58%)] bg-[hsl(255,75%,58%,0.03)]" : "border-border bg-secondary/30"} relative`}>
                              {plan.badge && (
                                <span className="absolute -top-2 left-3 px-1.5 py-0.5 rounded text-[9px] font-bold text-white" style={{ background: "hsl(255,75%,58%)" }}>{plan.badge}</span>
                              )}
                              <p className="font-bold text-xs text-foreground">{plan.name}</p>
                              <p className="mt-1">
                                <span className="text-sm font-black text-foreground">{plan.price}</span>
                                {plan.unit && <span className="text-[10px] text-muted-foreground ml-0.5">{plan.unit}</span>}
                              </p>
                              <ul className="mt-2 space-y-1">
                                {(plan.features as any[]).slice(0, 2).map((f: any, fi: number) => (
                                  <li key={fi} className="flex items-start gap-1">
                                    <Check className="w-2.5 h-2.5 mt-0.5 shrink-0 text-green-600" />
                                    <span className="text-[10px] text-muted-foreground">{f.main}</span>
                                  </li>
                                ))}
                              </ul>
                            </div>
