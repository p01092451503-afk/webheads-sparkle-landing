...
function AnimatedStat({ value, label, sub }: { value: string; label: string; sub: string }) {
  ...
  const { ref, value: animVal } = useCountUp(num, 1800);
  ...
  return (
    <div ref={ref} className="flex flex-col items-center justify-center py-8 px-4 text-center">
      ...
      >
        {num > 0 ? `${animVal.toLocaleString()}${suffix}` : value}
      </span>
      <span className="block text-sm font-semibold text-foreground mb-0.5">{label}</span>
...
export default function LmsPage() {
  ...
  return (
    <div className="min-h-screen" style={{ background: "var(--lms-page-bg)", fontFamily: "'Noto Sans KR', 'Pretendard Variable', 'Pretendard', sans-serif" }}>
      ...
      <section className="relative flex items-center justify-center pt-44 pb-36 overflow-hidden">
        ...
        <div className="container mx-auto px-6 relative z-10 text-center flex flex-col items-center">
          <span
            ...
            style={{ background: "rgba(255,255,255,0.12)", backdropFilter: "blur(8px)", color: "white", border: "1px solid rgba(255,255,255,0.15)" }}
          >
            <Sparkles className="w-3.5 h-3.5" />
            {t("lms.hero.badge")}
          </span>
          <h1
            className="text-4xl lg:text-[3.5rem] font-bold leading-[1.12] mb-5 tracking-tight text-white"
          ...
          >
            ...
            <span
              style={{
                background: "linear-gradient(135deg, hsl(0,0%,100%) 0%, hsl(210,100%,85%) 50%, hsl(260,80%,80%) 100%)",
      ...
      {/* ═══ Wave Divider (LMS-exclusive) ═══ */}
      <LmsWaveDivider colorFrom="hsl(var(--background))" />
      ...
      {/* ═══ Cloud (AI) vs On-premise (NEO) vs SaaS Comparison ═══ */}
      <LmsWaveDivider flip colorFrom="hsl(var(--secondary))" />
      <Reveal>
        <section id="solutions" className="py-28 bg-secondary">
          <div className="container mx-auto px-6 max-w-7xl">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              {/* Cloud AI */}
              <Reveal delay={0}>
                <div className="rounded-3xl p-8 flex flex-col h-full transition-all duration-300 hover:-translate-y-2 hover:shadow-xl"
                  style={{
                    background: "rgba(255,255,255,0.85)",
                    backdropFilter: "blur(12px)",
                    border: "1px solid hsl(var(--border) / 0.5)",
                    boxShadow: "0 4px 30px -8px hsl(var(--foreground) / 0.06)",
                  }}
                >
              ...
              <Reveal delay={300}>
                ...
                >
                  ...
                  <a href="#contact" className="block text-center mt-8 py-3.5 rounded-full font-bold text-sm transition-all duration-200 text-white hover:opacity-90 hover:shadow-lg" style={{ background: "hsl(170, 55%, 38%)" }}>
                    {t("lms.saas.cta")}
                  </a>
                </div>
              </Reveal>
            ...
            <Reveal>
              <div className="mt-12">
                ...
                <div className="rounded-2xl border border-border overflow-hidden bg-background" style={{ boxShadow: "0 4px 24px -8px hsl(255 60% 50% / 0.06)" }}>
              ...
              {allInOneFeatures.map((f: any, i: number) => {
                ...
                return (
                  <Reveal key={f.title} delay={i * 80}>
                    <div
                      className="rounded-2xl p-7 transition-all duration-300 flex flex-col gap-3 hover:shadow-lg hover:-translate-y-1"
                      style={{
      ...
      {/* ═══ AI Features — Premium gradient accent (LMS-exclusive) ═══ */}
      <Reveal>
        <section className="py-28 relative overflow-hidden">
          {/* Subtle gradient accent behind section */}
          <div className="absolute top-0 right-0 w-1/2 h-full pointer-events-none" style={{
            background: "radial-gradient(ellipse at 80% 30%, hsl(255 75% 58% / 0.04) 0%, transparent 60%)",
          }} />
              ...
              {aiFeatures.map((f: any, i: number) => {
                ...
                return (
                  <Reveal key={f.title} delay={i * 100}>
                    <div className="rounded-2xl p-7 transition-all duration-300 flex flex-col gap-3 hover:shadow-lg hover:-translate-y-1 relative overflow-hidden"
                      style={{
      ...
      {/* ═══ Plans ═══ */}
      <LmsWaveDivider flip colorFrom="var(--lms-page-bg)" />
      <Reveal>
        <section id="plans" className="py-28" style={{ background: "var(--lms-section-alt)" }}>
          <div className="container mx-auto px-6 max-w-6xl">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5 items-start">
              {(t("lms.plans", { returnObjects: true }) as any[]).map((plan: any, planIdx: number) => (
                <Reveal key={plan.name} delay={planIdx * 100}>
                  <div className={`relative rounded-3xl flex flex-col gap-0 transition-all duration-300 overflow-hidden ${plan.highlight ? "bg-background shadow-xl scale-[1.02]" : "bg-background border border-border hover:border-muted-foreground/30 hover:shadow-md"}`} style={plan.highlight ? { border: `2px solid hsl(var(--lms-primary))` } : undefined}>
                    {plan.badge && (
