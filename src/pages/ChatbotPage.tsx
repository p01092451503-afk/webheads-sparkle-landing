...
const features = [
  ...
  { icon: Globe, title: "다국어 처리 & 번역", desc: "한국어, 영어, 중국어, 일본어 등 10개 이상의 언어를 자동 감지하고 해당 언어로 응답합니다.", tags: ["한/영/중/일", "자동 언어 감지", "10개 이상 지원", "글로벌"] },
  { icon: Link2, title: "멀티채널 배포", desc: "카카오톡 채널, 웹사이트 채팅 위젯, 모바일 앱 내 SDK 형태로 단일 챗봇을 다채널에 동시 배포합니다.", tags: ["카카오톡", "웹 위젯", "모바일 SDK", "세션 관리"] },
  { icon: BarChart3, title: "운영 현황 대시보드", desc: "질문 유형 분류, 해결률, 에스컬레이션 비율, 사용자 만족도(CSAT), 평균 응답 시간 등 KPI를 실시간 대시보드로 시각화합니다.", tags: ["실시간 모니터링", "CSAT", "KPI", "시각화"] },
  { icon: ShieldCheck, title: "보안 & 개인정보 보호", desc: "학습자 개인정보를 암호화 전송(TLS 1.3)하고, 대화 로그는 사용자 동의 기반으로 수집합니다. 개인정보보호법 및 GDPR 가이드라인을 준수합니다.", tags: ["TLS 1.3", "암호화", "GDPR", "개인정보보호법"] },
  { icon: RefreshCw, title: "지속 학습 & 모델 고도화", desc: "실제 대화 데이터를 기반으로 주기적 파인튜닝을 수행하여 시간이 지날수록 응답 정확도가 향상됩니다.", tags: ["Fine-tuning", "RLHF", "피드백 루프", "자동 고도화"] },
  { icon: Users, title: "상담원 에스컬레이션 & 협업", desc: "챗봇이 처리하기 어려운 복잡한 문의는 실시간으로 상담원에게 전환하며, 이전 대화 맥락을 그대로 인계합니다.", tags: ["Human Handoff", "실시간 전환", "맥락 인계", "협업"] },
];

const process = [
  { step: "01", title: "요구사항 분석 & 설계", desc: "기관 특성, 학습자 문의 패턴, 연동 시스템을 심층 분석하여 챗봇 아키텍처와 대화 시나리오를 설계합니다.", detail: "인텐트 설계 · 엔티티 정의 · 아키텍처 설계" },
  { step: "02", title: "데이터 수집 & AI 학습", desc: "FAQ, 강의 자료, 내부 규정 등 기관 보유 문서를 수집·정제하고 벡터 임베딩으로 변환합니다.", detail: "문서 파싱 · 벡터 DB 구축 · RAG 파이프라인" },
  { step: "03", title: "개발 & 품질 검증", desc: "챗봇 엔진 개발 및 LMS·채널 연동 작업 후 100개 이상의 엣지케이스 시나리오를 테스트합니다.", detail: "시나리오 테스트 · 성능 측정 · Fallback 검증" },
  { step: "04", title: "배포 & 지속 운영 지원", desc: "전 채널 배포 후 월별 성능 리포트와 정기 파인튜닝으로 챗봇 품질을 지속 개선합니다.", detail: "멀티채널 배포 · 월별 리포트 · 정기 개선" },
];
...
const faqs = [
  { q: "챗봇 도입 후 기존 FAQ를 바로 사용할 수 있나요?", a: "네, 기존 FAQ 문서를 업로드하면 벡터 임베딩으로 자동 색인되어 즉시 사용할 수 있습니다. Excel, PDF, Word 등 다양한 형식을 지원합니다." },
  { q: "챗봇이 답변하지 못하는 질문은 어떻게 처리되나요?", a: "Fallback 시나리오에 따라 '상담원 연결' 또는 '문의 접수' 옵션을 자동 제안합니다. 상담원 전환 시 이전 대화 맥락이 그대로 인계됩니다." },
  { q: "카카오톡 채널과 웹사이트에 동시에 적용할 수 있나요?", a: "네, 단일 챗봇 엔진으로 카카오톡, 웹사이트, 모바일 앱에 동시 배포 가능합니다. 채널별 대화 맥락도 통합 관리됩니다." },
  { q: "학습자 개인정보는 안전하게 보호되나요?", a: "TLS 1.3 암호화 전송, 개인정보 마스킹, 동의 기반 로그 수집 등 개인정보보호법과 GDPR 가이드라인을 준수합니다." },
  { q: "챗봇 응답 정확도는 어떻게 관리되나요?", a: "월별 정기 파인튜닝과 피드백 루프를 통해 지속적으로 개선합니다. 대시보드에서 해결률과 CSAT 지표를 실시간으로 모니터링할 수 있습니다." },
];

export default function ChatbotPage() {
  return (
    <div className="min-h-screen bg-background">
      <SEO title="AI 챗봇" description="GPT-4o, Claude 3.5 기반 LMS 연동 AI 챗봇. 학습자 문의 자동 응대, FAQ 자동 학습, 다국어 지원으로 24시간 학습 지원 환경을 구축합니다." keywords="AI 챗봇, LMS 챗봇, 이러닝 챗봇, GPT 챗봇, 학습 챗봇, 이러닝 상담봇" path="/chatbot" jsonLd={{ "@context": "https://schema.org", "@type": "Service", "name": "웹헤즈 AI 챗봇", "provider": { "@type": "Organization", "name": "웹헤즈" }, "description": "GPT-4o, Claude 3.5 기반 LMS 연동 AI 챗봇 서비스", "areaServed": "KR", "serviceType": "AI 챗봇 개발", "url": "https://webheads-sparkle-landing.lovable.app/chatbot" }} />
      ...
      <section className="relative min-h-[76vh] flex items-center pt-20 pb-14 overflow-hidden" style={{ background: "linear-gradient(160deg, hsl(210, 50%, 92%) 0%, hsl(214, 60%, 88%) 40%, hsl(220, 50%, 85%) 100%)" }}>
        ...
        <div className="container mx-auto px-6 py-24 relative z-10 lg:pl-[10%]">
          <div className="max-w-xl">
            ...
            <h1 className="text-4xl lg:text-[3.2rem] font-black leading-tight mb-5 tracking-tight" style={{ color: "hsl(220, 60%, 8%)" }}>이러닝 전문<br /><span style={{ color: "hsl(192, 90%, 42%)" }}>AI 챗봇 솔루션</span></h1>
      ...
      <section className="py-24 bg-background border-b border-border">
        <div className="container mx-auto px-6 max-w-5xl">
          <div className="grid grid-cols-3 gap-0 divide-x divide-border">
            {stats.map((s) => (<div key={s.label} className="flex flex-col items-center justify-center py-8 px-4 text-center"><span className="block font-black leading-none mb-2 text-5xl md:text-6xl text-foreground tracking-tight">{s.value}</span><span className="block text-sm font-semibold text-foreground mb-0.5">{s.label}</span><span className="block text-xs text-muted-foreground">{s.sub}</span></div>))}
          </div>
        </div>
      </section>
      ...
      <ServiceMidCTA heading="우리 기관에 맞는 AI 챗봇이 궁금하신가요?" description="기관 특성에 맞는 챗봇 구축 방안을 무료로 제안드립니다." ctaText="무료 데모 신청하기" />
