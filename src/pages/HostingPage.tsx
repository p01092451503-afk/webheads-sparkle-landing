...
export default function HostingPage() {
  ...
  return (
    <div className="min-h-screen bg-background">
      <SEO title={t("hosting.seo.title")} description={t("hosting.seo.description")} keywords={t("hosting.seo.keywords")} path="/hosting" breadcrumb={[{ name: t("hosting.seo.title"), url: `${BASE_URL}/hosting` }]} jsonLd={{ "@context": "https://schema.org", "@type": "Service", "name": `${t("hosting.seo.title")} - Webheads`, "provider": { "@type": "Organization", "name": "Webheads (웹헤즈)" }, "description": t("hosting.seo.description"), "areaServed": "KR", "serviceType": t("hosting.seo.title"), <SEO title={t("hosting.seo.title")} description={t("hosting.seo.description")} keywords={t("hosting.seo.keywords")} path="/hosting" breadcrumb={[{ name: t("hosting.seo.title"), url: `${BASE_URL}/hosting` }]} jsonLd={{ "@context": "https://schema.org", "@type": "Service", "name": `${t("hosting.seo.title")} - Webheads`, "provider": { "@type": "Organization", "name": "Webheads (웹헤즈)" }, "description": t("hosting.seo.description"), "areaServed": "KR", "serviceType": t("hosting.seo.title"), "url": `${BASE_URL}/hosting`, "additionalProperty": { "@type": "PropertyValue", "name": "clientCount", "value": "300" } }} faqJsonLd={faqs} howToJsonLd={{ name: "이러닝 호스팅 도입 절차", description: "CDN 기반 동영상 스트리밍 호스팅을 도입하는 단계별 프로세스입니다.", steps: [{ name: "현황 분석", text: "현재 서버 환경, 트래픽 규모, 콘텐츠 용량을 분석합니다." }, { name: "인프라 설계", text: "CDN, 스토리지, 인코딩 파이프라인, 보안 정책을 설계합니다." }, { name: "마이그레이션", text: "기존 콘텐츠를 새 인프라로 이관하고 DNS를 전환합니다." }, { name: "최적화 및 모니터링", text: "성능 최적화 후 24/7 모니터링 체계를 구축합니다." }] }} />={t("hosting.seo.title")} description={t("hosting.seo.description")} keywords={t("hosting.seo.keywords")} path="/hosting" breadcrumb={[{ name: t("hosting.seo.title"), url: `${BASE_URL}/hosting` }]} jsonLd={{ "@context": "https://schema.org", "@type": "Service", "name": `${t("hosting.seo.title")} - Webheads`, "provider": { "@type": "Organization", "name": "Webheads (웹헤즈)" }, "description": t("hosting.seo.description"), "areaServed": "KR", "serviceType": t("hosting.seo.title"), "url": `${BASE_URL}/hosting`, "additionalProperty": { "@type": "PropertyValue", "name": "clientCount", "value": "300" } }} faqJsonLd={faqs} howToJsonLd={{ name: "이러닝 호스팅 도입 절차", description: "CDN 기반 동영상 스트리밍 호스팅을 도입하는 단계별 프로세스입니다.", steps: [{ name: "현황 분석", text: "현재 서버 환경, 트래픽 규모, 콘텐츠 용량을 분석합니다." }, { name: "인프라 설계", text: "CDN, 스토리지, 인코딩 파이프라인, 보안 정책을 설계합니다." }, { name: "마이그레이션", text: "기존 콘텐츠를 새 인프라로 이관하고 DNS를 전환합니다." }, { name: "최적화 및 모니터링", text: "성능 최적화 후 24/7 모니터링 체계를 구축합니다." }] }} /> faqJsonLd={faqs} howToJsonLd={{ name: "이러닝 호스팅 도입 절차", description: "CDN 기반 동영상 스트리밍 호스팅을 도입하는 단계별 프로세스입니다.", steps: [{ name: "현황 분석", text: "현재 서버 환경, 트래픽 규모, 콘텐츠 용량을 분석합니다." }, { name: "인프라 설계", text: "CDN, 스토리지, 인코딩 파이프라인, 보안 정책을 설계합니다." }, { name: "마이그레이션", text: "기존 콘텐츠를 새 인프라로 이관하고 DNS를 전환합니다." }, { name: "최적화 및 모니터링", text: "성능 최적화 후 24/7 모니터링 체계를 구축합니다." }] }} />
      ...
      <section className="py-28 bg-secondary">
        <div className="container mx-auto px-6 max-w-5xl">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
            {features.map((f: any) => (<div key={f.title} className="group rounded-2xl p-7 bg-background hover:bg-muted transition-colors duration-200 flex flex-col gap-3 border border-border hover:shadow-md"><div className="w-10 h-10 rounded-xl flex items-center justify-center bg-secondary shadow-sm"><f.icon className="w-5 h-5 text-primary" /></div><h3 className="font-bold text-foreground text-base tracking-tight">{f.title}</h3><p className="text-muted-foreground text-sm leading-relaxed flex-1">{f.desc}</p><div className="flex flex-wrap gap-1.5 mt-1">{f.tags.map((tag: string) => (<span key={tag} className="text-xs px-2.5 py-1 rounded-full font-medium bg-primary/10 text-primary">{tag}</span>))}</div></div>))}
          </div>
        </div>
      </section>
      ...
      {/* Comparison - W */}
      <ServiceComparison bg="bg-background" headers={comparisonHeaders} rows={comparisonRows} subheading={t("hosting.comparisonSection.sub")} heading={t("hosting.comparisonSection.heading")} description={t("hosting.comparisonSection.desc")} />
      ...
      <section id="plans" className="py-28" style={{ background: "var(--plans-bg)" }}>
        <div className="container mx-auto px-6 max-w-6xl">
          ...
          <div className="mt-8 flex flex-col sm:flex-row items-center justify-center gap-3 sm:gap-6 rounded-xl border border-border bg-muted px-6 py-4">
            <p className="text-foreground font-semibold text-sm">{t("hosting.plansCustom.title")} <span className="font-normal text-muted-foreground">{t("hosting.plansCustom.desc")}</span></p>
            <a href="#contact" className="shrink-0 inline-flex px-5 py-2.5 rounded-2xl font-bold text-xs bg-primary text-primary-foreground hover:bg-primary/90 transition-colors">{t("hosting.plansCustom.cta")}</a>
          </div>
        </div>
      </section>

      {/* CaseStudy - W */}
      <ServiceCaseStudy bg="bg-background" cases={caseStudies} subheading={t("hosting.caseStudySection.sub")} heading={t("hosting.caseStudySection.heading")} description={t("hosting.caseStudySection.desc")} />

      {/* Process - G */}
      <ServiceProcess bg="bg-secondary" steps={processSteps} subheading={t("hosting.processSection.sub")} heading={t("hosting.processSection.heading")} description={t("hosting.processSection.desc")} />
