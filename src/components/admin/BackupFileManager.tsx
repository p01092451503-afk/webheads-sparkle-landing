...
export default function BackupFileManager({ isSuperAdmin }: { isSuperAdmin: boolean }) {
  ...
  const fetchFiles = useCallback(async () => {
    ...
    if (error) {
      ...
      const uniqueFolders = [...new Set([...DEFAULT_FOLDERS, ...((data as any[]) || []).map((f: BackupFile) => f.folder)])];
  ...
  const handleUpload = async () => {
    ...
    try {
      for (const file of selectedFiles) {
        ...
        const { error: dbError } = await (supabase.from("backup_files" as any) as any).insert({
          file_name: file.name,
          file_path: filePath,
          file_size: file.size,
  ...
  const handleDelete = async () => {
    ...
    try {
      await supabase.storage.from("backup-files").remove([deleteTarget.file_path]);
      await (supabase.from("backup_files" as any) as any).delete().eq("id", deleteTarget.id);
  ...
  const handleDownload = async (file: BackupFile) => {
    const { data, error } = await supabase.storage.from("backup-files").download(file.file_path);
  ...
  const handlePreview = async (file: BackupFile) => {
    const { data, error } = await supabase.storage.from("backup-files").download(file.file_path);
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <h2 className="text-lg font-bold flex items-center gap-2">
          <HardDrive className="w-5 h-5" /> 자료백업
        </h2>
      ...
      <Dialog open={uploadOpen} onOpenChange={setUploadOpen}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>자료백업 파일을 업로드합니다.</DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
            <div>
              <Label>폴더</Label>
