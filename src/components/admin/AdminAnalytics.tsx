      ...
      <SectionGroup title="방문자 유형 분석" number={2}>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {[
            ...
            { icon: <Shield className="w-4 h-4" />, label: "스크래퍼", value: visitorTypeCounts.scraper, color: "hsl(0, 70%, 55%)", pct: visitorTypeCounts.total > 0 ? Math.round((visitorTypeCounts.scraper / visitorTypeCounts.total) * 100) : 0, tooltip: "SEO 분석 도구(Ahrefs, SEMrush 등), Cloudflare 프록시, 반복 접속 스크래퍼 등의 방문입니다. 실제 사용자가 아니므로 통계 왜곡의 원인이 됩니다." },
