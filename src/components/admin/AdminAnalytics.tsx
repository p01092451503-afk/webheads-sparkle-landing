    ...
    <div className="flex flex-col gap-5">
      ...
      <SectionGroup title="주요 지표 요약" number={1}>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          ...
          <MetricCard icon={<Link2 className="w-[18px] h-[18px]" />} label="UTM 유입" value={utmSourceCounts.reduce((s, [, c]) => s + c, 0)} color="hsl(170, 70%, 40%)" sub={utmSourceCounts.length > 0 ? utmSourceCounts.slice(0, 3).map(([s, c]) => `${s} ${c}`).join(", ") : undefined} tooltip="UTM 파라미터 유입 수입니다." />
        </div>
      </SectionGroup>
